...
export async function handleTasksPending(globals: GlobalFlags, argv: any) {
  ...
  try {
    const rawPendingTasks = await getPendingTasks(classroom, globals);
    const now = new Date();
    const range = resolveDateRange(globals.from, globals.last);
    const pendingTasks = applyDateFilter(rawPendingTasks, range, (t: any) => {
      if (t.courseWork.dueDate) return parseDueDate(t.courseWork);
      return t.courseWork.updateTime || t.courseWork.creationTime;
    });

    pendingTasks.sort((a, b) => {
      const aDate = a.courseWork.dueDate ? parseDueDate(a.courseWork).getTime() : Infinity;
      const bDate = b.courseWork.dueDate ? parseDueDate(b.courseWork).getTime() : Infinity;
      return aDate - bDate;
    });
...
export async function handleTopic(verb: string | undefined, globals: GlobalFlags, argv: any) {
  ...
  if (verb === 'list') {
    ...
    const range = resolveDateRange(globals.from, globals.last);
    const allCw = applyDateFilter(cwRes.data.courseWork || [], range, (cw: any) => cw.dueDate ? parseDueDate(cw) : cw.updateTime);
    const allMat = applyDateFilter(matRes.data.courseWorkMaterial || [], range, (m: any) => m.updateTime);
